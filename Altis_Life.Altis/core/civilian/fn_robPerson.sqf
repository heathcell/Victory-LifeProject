...
if (CASH > 0) then {
    ...
    if (life_HC_isActive) then {
        [_robber getVariable ["userid",name _robber],_robber getVariable ["realname",name _robber],"211"] remoteExecCall ["HC_fnc_wantedAdd",HC_Life];
    } else {
        [_robber getVariable ["userid",name _robber],_robber getVariable ["realname",name _robber],"211"] remoteExecCall ["life_fnc_wantedAdd",RSERV];
    };
