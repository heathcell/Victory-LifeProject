...
if (count _invs > 0) then {
    ...
    if (_illegal > 6000) then {
        ...
        if (life_HC_isActive) then {
            [_civ getVariable ["userid",name _civ],_civ getVariable ["realname",name _civ],"482"] remoteExecCall ["HC_fnc_wantedAdd",HC_Life];
        } else {
            [_civ getVariable ["userid",name _civ],_civ getVariable ["realname",name _civ],"482"] remoteExecCall ["life_fnc_wantedAdd",RSERV];
        };

    };

    if (life_HC_isActive) then {
        [_civ getVariable ["userid",name _civ],_civ getVariable ["realname",name _civ],"481"] remoteExecCall ["HC_fnc_wantedAdd",HC_Life];
    } else {
