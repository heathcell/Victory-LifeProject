...
{if(_x getVariable ["userid",name _x] == _seller) then {_online = true; _unit = _x};} foreach allPlayers;

if (_online) then {
	[1,_id] remoteExecCall ["TON_fnc_vAH_update",RSERV];
	...
	[3,_id,_value,_seller] remoteExecCall ["TON_fnc_vAH_update",RSERV];
