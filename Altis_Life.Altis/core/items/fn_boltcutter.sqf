...
if (((nearestObject [_pos,"Land_Dome_Big_F"]) == _building || (nearestObject [_pos,_vaultHouse]) == _building) && (independent countSide playableUnits < (LIFE_SETTINGS(getNumber,"minimum_cops")))) exitWith {
...
if (life_HC_isActive) then {
    [USERID,profileName,"459"] remoteExecCall ["HC_fnc_wantedAdd",HC_Life];
} else {
    [USERID,profileName,"459"] remoteExecCall ["life_fnc_wantedAdd",RSERV];
};
