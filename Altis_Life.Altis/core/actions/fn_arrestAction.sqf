...
if (!((side _unit) in [civilian,east])) exitWith {}; //Not a civ
if (isNull _unit) exitWith {}; //Not valid

if (life_HC_isActive) then {
    [_unit getVariable ["userid",name _unit],_unit,player,false] remoteExecCall ["HC_fnc_wantedBounty",HC_Life];
} else {
    [_unit getVariable ["userid",name _unit],_unit,player,false] remoteExecCall ["life_fnc_wantedBounty",RSERV];
};
